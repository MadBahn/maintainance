...
<div class="section-box" style="height: 890px;">
    ...
    <div class="add" id="al" style="display: none">
        <form method="get" action="line">
            ...
            <br>
            <textarea name="remark" style="width: 300px;height: 50px;"></textarea>
            <br>
...
<%--        <button id="ee">修改</button>--%>
        <form method="get" action="${pageContext.request.contextPath}/line" id="ee_f" style="padding: 10px;border: 1px solid;">
            ...
            <span class="abel">备注：</span>
            <br>
            <textarea name="remark" style="width: 300px;height: 50px;"><%=i.getRail_remark()%></textarea>
            <br>
            <input type="submit" value="修改">
        </form>
        <button onclick="del('<%=i.getRail_id()%>')">删除</button>
        <br>
        <%}%>
    </div>
</div>
...
<script>
    function del(id) {
        ...
        if(confirm("此操作不可逆转，是否继续删除"+id+"？")){
            window.location.href = "${pageContext.request.contextPath}/line?method=del&id="+id;
        }
    }
</script>
</html>
